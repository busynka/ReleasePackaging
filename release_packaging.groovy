...
if (args.size()<2) {println "directory source_file conf_file"}
else {zip(args[0], args[1], args[2])}
}
static void zip(String dir, String source_file, String conf_file){
    ...
    while(e.hasMoreElements()){
        ...
        if ((entryName =~ project)){  
            ...
            if (schedule!=-1){ProjectList.add(schedule)}      
        } 
        if ((entryName =~ branch)){  
    ...
    conf.eachLine{line, lineNumber->
        ...
        else if (lineNumber > 2) {emptyList.add(Arrays.toString(line.split(delims)))}                                   // read the rest of the file
    }
        ...
        uniqueBranchList.each{i->                                                                                       // loop through unique branch names
            emptyList.eachWithIndex{p,c ->                                                                              // parse the list and add it to a map
                ...
                if((list[c].project==temp)&&(i==list[c].branch)){                                                   	// check if there is configuration for that branch and that project in the properties file
                    exclude=exclude+",**/"+list[c].exclude+"/"+"**"                                                     // add the values that you want to exclude if the previos condition is true   
                    //include=include+",**/"+list[c].include+"/"+"**"
                    }
                ant.zip(destfile: destFile){zipfileset(src:path,excludes:exclude,includes:include)}                     // create zips
